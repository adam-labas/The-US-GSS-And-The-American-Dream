...
If you ask a group of first graders what they aspire to be when they grow up, it wouldn't be surprising to hear a lot of kids say that they would want to be the richest person in the world. For as long as humanity has existed, the allure and pride associated with wealth and the accumulation of large sums of wealth has always been at the forefront of the social and economic paradigm. Whether it was Mansa Musa in East Africa in the 14th century or Jeff Bezos today, the wealthy have always stood out in large crowds. This aspect of the social sphere is one that can undoubtedly be found in American livelihood. It would be hard fro any of ud to deny that money rules the world, regardless of how we feel about that. We need it to put food on our tables, clothes on our backs and seemingly to increase our quality of life. As such, it would appear as if a surplus of money in ones possession could do no harm.

For the last 50 years, people from all over the world have began to flock to the United States as immigrants in search of the so called American Dream. The American Dream is a Utopian idea in which the United States of America is a safe haven for all those looking to improve their quality of life, via monetary gain. It implies that anyone (yes, absolutely anyone) who is willing to put in the hard work required to be successful, will have much improves abilities to make something of themselves in the United States than they could elsewhere, all else being equal.

In this analysis, we got our hands on some of the 2021 United States General Social Survey data which is described with great extent in the Data Section \@ref(data). In our analysis, we decided to focus on  the various factors and trends in the data in order to try and gather some implications on the veracity of the American Dream. To further break down the analysis, we decided to look into the distribution of the aforementioned data by segregating the data not only by the current income of respondents but also by their income at age 16, and trying to see if we can spot any relation between an increase in income from age 16 to now and the gender, or the number of grandparents of the respondent, born in the United States. 

In this paper, we will attempt to take you on a journey to explore the data that was used to produce the findings, we will visually present to you the findings and we will speak about the implications of the findings and later on in the Limitations Section \@ref(limitations), we will discuss the limitations of our data and problems that this may have, when wanting to extract true, ethical, unbiased information from the implications of the findings.
...
```{r tableofdata, echo = F, fig.pos = "!H"}
#table of variables that i am concerned with
data_table <- reduced_data[,-c(1,6,8)]

data_table[complete.cases(data_table), ] %>%
   slice(1:7) %>%
  kable(
    caption = "GSS Cleaned Data",
    col.names = c("Age", "Gender", "Born", 
                  "Education", 
                  "Birth Year", "Grandparents", 
                  "Income at 16","Current Income"),
    digits = 1,
    booktabs = TRUE,  
    linesep = "") %>% 
...
## Visual 1
```{r figure-1, fig.cap="write a figure caption here", echo = FALSE, fig.width=11, fig.height=9, message = F, warning = F}
...
```{r figure2, fig.cap="Comparing proportions of current income and income at age 16, grouping by gender", echo = FALSE, fig.width=12, fig.height=10, message = F, warning = F}
...
```{r, fig.cap="Comparing proportions of current income and income at age 16, grouping by number of US born grandparents", echo = FALSE, fig.width=12, fig.height=10, message = F, warning = F}
...
In a similar fashion to what was explained in Section \@ref(implications-1), both of our graphs are heavily skewed towards higher income. In the case of current income, it is almost certainly because of the bin width decided on by the people who built the survey which we personally find many problems with. In Section \@ref(limitations), we will take a deep dive into this.


## Visuals 3
```{r, echo=FALSE, fig.cap="Comparing proportions of current income and income at age 16, grouping by US born grandparents or not", fig.height=10, fig.width=12, message=FALSE, warning=FALSE}
...
## Uneven Bin width for Income Groups.
The first major flaw I've found in the data is that the bins that both the of the current income variable, the the first bin has a size of \$1,000, the second bin has a width of \$1,999, the third returns to about \$1,000 and as we go higher up the echelon, we have seemingly random alterations in bin widths from \$1,000 to \$2,000 to \$5,000. This can cause serious problems in statistics and especially for purposes of analysis. 
...
## Why cut off income at $25,000?
According to a news release published by the United States Bureau of Labor Statistics (see the document called wkyeng.pdf in the literature folder in inputs), the median weekly income of US citizens in the 4th quarter of 2021 (furthest from the start of the pandemic) is $1,010. This information is from that of 116.3 million full-time wage and salary workers. If we do some basic mathematics to approximate the annual earning, we get a figure of over \$52,000 dollars per year. As such, we are unable to understand why the bins for current income have been designed as such. This is a major flaw in the technique used by the surveyors to collect the data. In section \@ref(next-steps), we will discuss ways that the survey could improve on this.

## Small sample size
As we have reported in Section \@ref(key-features-pros-and-cons-of-the-methodology-and-survey), out of the 27,591 lines of sample (people who had been prompted to answer the questionnaire), 4,032 responses were collected. This means that for the 2021 GSS, the response rate was at 17.4 percent (see calculation on page 28 of the "GSS 2021 Codebook R1b.pdf"). The United States has a population of over 330 million and even if all the people who had been prompted to answer the survey had responded it, the survey would only represent less than $8.36 \times 10^{-3}$ percent of the population. It is quite reasonable that as such, it would be hard to make generalizations for the entire adult united states population.

## Next steps
Following on from the latter section, we can make some pretty good suggestions on ways to improve the survey and subsequently, the data and the implications of the findings, so that they will be less ambiguous and more ethical to use with confidence. First off, having the data in predetermined bins is problematic from the perspective of the data analyst. We can start by creating even bins that respondents can answer but we can also allow the respondents to provide their exact income. The latter solution will give us more data, and it gives more options to the data analysis to conduct a reproducible analysis. In this manner, the data scientist who gets a hold of the data will have the liberty to conduct his analysis and chose the bins which work best for the task he is working on, instead of being obstructed by barriers built into the data. To account for some of these changes, we have created a supplementary survey which collects a lot of the data we were interested in for this analysis. It stands independently from the 2021 US GSS. See the Appendix for the survey and additional information.
...
# Appendix {-}
Below is a link and a QR code for the supplementary survey alluded to in Section \@ref(next-steps).


LINK TO SURVEY: https://docs.google.com/forms/d/1a3tK-uSkl0MDrjqxWW1aCKhLowQxdqBvE6Bid6wN3YY/edit?ts=6239fa2b

![Supplimentary Survey](~/paper_3_team_paper/inputs/Supplimentary_survey_QR.png){width="300" height="500"}
